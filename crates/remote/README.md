...
The `remote` crate contains the implementation of the openteams hosted API.
...
```env
AGENT_CHATGROUP_REMOTE_JWT_SECRET=your_base64_encoded_secret
...
Generate `AGENT_CHATGROUP_REMOTE_JWT_SECRET` once using `openssl rand -base64 48` and copy the value into `.env.remote`.

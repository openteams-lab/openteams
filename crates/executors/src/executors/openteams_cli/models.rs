...
use crate::executors::openteams_cli::{
    sdk::EventStreamContext,
    types::{MessageRole, OpenTeamsCliExecutorEvent, ProviderListResponse, SdkEvent},
};
...
pub(super) async fn maybe_emit_token_usage(context: &EventStreamContext<'_>, event: &Value) {
    ...
    let _ = context
        .log_writer
        .log_event(&OpenTeamsCliExecutorEvent::TokenUsage {
            total_tokens,
...
async fn fetch_model_context_windows(
        ...
        .await
    {
        Ok(resp) => resp,
        Err(err) => {
            tracing::debug!("OpenTeamsCli provider list request failed: {err}");
            return None;
        }
    };
    ...
    if !response.status().is_success() {
        tracing::debug!(
            "OpenTeamsCli provider list request failed with HTTP {}",
            response.status()
        );
        return None;
    }
    ...
    let parsed = match response.json::<ProviderListResponse>().await {
        Ok(parsed) => parsed,
        Err(err) => {
            tracing::debug!("OpenTeamsCli provider list response parse failed: {err}");
            return None;
        }
    };

//! OpenTeamsCli slash command parsing, execution, and result formatting.
//!
//! This module is the central place for all slash command handling in OpenTeamsCli.
...
/// OpenTeamsCli slash command with known variants and custom fallback.
#[derive(Debug, Clone)]
pub enum OpenTeamsCliSlashCommand {
    Compact,
    Commands,
    Models {
...
impl OpenTeamsCli {
    pub async fn discover_slash_commands(
        ...
        let key = SlashCommandCacheKey::new(current_dir, &BaseCodingAgent::OpenTeamsCli);
...
impl<'a> From<SlashCommandCall<'a>> for OpenTeamsCliSlashCommand {
...
/// Execute a slash command using the OpenTeamsCli SDK.
pub async fn execute(
    config: RunConfig,
    command: OpenTeamsCliSlashCommand,
    ...
    match &command {
        OpenTeamsCliSlashCommand::Commands => {
        ...
        OpenTeamsCliSlashCommand::Models { provider } => {
        ...
        OpenTeamsCliSlashCommand::Agents => {
        ...
        OpenTeamsCliSlashCommand::Status => {
        ...
        OpenTeamsCliSlashCommand::Mcp => {
            let mcp = sdk::mcp_status(&client, &config.base_url, &config.directory).await?;
            log_result_and_done(&log_writer, format_mcp(&mcp)).await?;
            return Ok(());
        }
        // Session-dependent commands handled below
        OpenTeamsCliSlashCommand::Compact | OpenTeamsCliSlashCommand::Custom { .. } => {}
    }

    // Validate custom commands exist
    if let OpenTeamsCliSlashCommand::Custom { name, .. } = &command {
    ...
    let session_id = match config.resume_session_id.as_deref() {
        Some(existing) if command.should_fork_session() => {
            tokio::select! {
                _ = cancel.cancelled() => return Ok(()),
                res = sdk::resolve_session_id(&client, &config.base_url, &config.directory, Some(existing)) => res?,
            }
        }
    ...
    let is_compact = matches!(&command, OpenTeamsCliSlashCommand::Compact);
    ...
    {
        OpenTeamsCliSlashCommand::Compact => {
            let model = compaction_model.ok_or_else(|| {
                ExecutorError::Io(io::Error::other("OpenTeamsCli compaction model missing"))
            })?;
            Box::pin(async move {
        ...
        OpenTeamsCliSlashCommand::Custom { name, arguments } => Box::pin(async move {
            sdk::session_command(
                &request_client,
                &request_base_url,
                &request_directory,

...
pub fn normalize_logs(msg_store: Arc<MsgStore>, worktree_path: &Path) {
    ...
    tokio::spawn(async move {
        ...
        while let Some(Ok(line)) = stdout_lines.next().await {
            ...
            match event {
                OpenTeamsCliExecutorEvent::StartupLog { .. } => {}
                OpenTeamsCliExecutorEvent::SessionStart { session_id } => {
                    if !stored_session_id {
                        msg_store.push_session_id(session_id);
                        stored_session_id = true;
                    }
                }
                OpenTeamsCliExecutorEvent::SdkEvent { event } => {
                    state.handle_sdk_event(&event, &worktree_path, &msg_store);
                }
                OpenTeamsCliExecutorEvent::TokenUsage {
                    total_tokens,
                ...
                OpenTeamsCliExecutorEvent::SlashCommandResult { message } => {
                ...
                OpenTeamsCliExecutorEvent::ApprovalResponse {
                    tool_call_id,
                    status,
                } => {
                ...
                OpenTeamsCliExecutorEvent::SystemMessage { content } => {
                    let idx = entry_index.next();
                ...
                OpenTeamsCliExecutorEvent::Error { message } => {
                    let idx = entry_index.next();
                    let (error_type, content) = if let Some(detected) =
                        crate::logs::api_errors::detect_api_error(&message)
                    {
                ...
                OpenTeamsCliExecutorEvent::Done => {}
            }
        }
    });
}

fn parse_event(line: &str) -> Option<OpenTeamsCliExecutorEvent> {
    serde_json::from_str::<OpenTeamsCliExecutorEvent>(line.trim()).ok()
}
...
impl LogState {
    ...
    fn handle_sdk_event(&mut self, raw: &Value, worktree_path: &Path, msg_store: &Arc<MsgStore>) {
        let Some(event) = SdkEvent::parse(raw) else {
            ...
            if !raw_text.trim().is_empty() {
                self.add_normalized_entry(system_message(format!(
                    "Unrecognized OpenTeamsCli SDK event: {raw_text}"
                )));
            }
            return;
        };
        ...
        match event {
            ...
            SdkEvent::SessionError(event) => {
                let (error_type, message) = match event.error {
                    Some(err) if err.kind() == "ProviderAuthError" => (
                        ...
                        err.message()
                            .unwrap_or_else(|| format!("OpenTeamsCli session error: {}", err.raw)),
                    ),
                    Some(err) => {
                        let raw_str = err.raw.to_string();
                        let raw_msg = format!("OpenTeamsCli session error: {}", err.raw);
                        if let Some(detected) = crate::logs::api_errors::detect_api_error(&raw_str)
                        {
                            (detected.error_type, detected.message)
                        } else {
                    ...
                    None => (
                        ...
                        "OpenTeamsCli session error".to_string(),
                    ),
                };
            ...
            SdkEvent::Unknown { type_, properties } => {
                self.add_normalized_entry(system_message(format!(
                    "Unrecognized OpenTeamsCli SDK event type `{type_}`: {properties}"
                )));
            }
        }
    }
    ...
    fn handle_session_status(&mut self, status: SessionStatus) {
        match status {
            SessionStatus::Retry {
                ...
                self.add_normalized_entry(system_message(format!(
                    "OpenTeamsCli retry (attempt {attempt}): {message} (next in {next}ms)"
                )));
            }
    ...
    fn handle_permission_asked(
        ...
        let Some(tool) = event.tool else {
            self.add_normalized_entry(system_message(format!(
                "OpenTeamsCli permission requested: {}",
                event.permission
            )));
            return;
        };

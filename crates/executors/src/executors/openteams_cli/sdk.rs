...
impl LogWriter {
    ...
    pub async fn log_event(&self, event: &OpenTeamsCliExecutorEvent) -> Result<(), ExecutorError> {
        let raw =
    ...
    pub async fn log_error(&self, message: String) -> Result<(), ExecutorError> {
        self.log_event(&OpenTeamsCliExecutorEvent::Error { message })
            .await
    }

    pub async fn log_slash_command_result(&self, message: String) -> Result<(), ExecutorError> {
        self.log_event(&OpenTeamsCliExecutorEvent::SlashCommandResult { message })
            .await
    }
...
/// Generate a cryptographically secure random password for OpenTeamsCli server auth.
...
async fn run_session_inner(
    ...
    log_writer
        .log_event(&OpenTeamsCliExecutorEvent::SessionStart {
        ...
        && !status.is_empty()
    {
        ...
        tracing::debug!("Sending commit reminder prompt to OpenTeamsCli session");

        // Log as system message so it's visible in the UI (user_message gets filtered out)
        let _ = log_writer
            .log_event(&OpenTeamsCliExecutorEvent::SystemMessage {
...
async fn prompt(
    ...
    if trimmed.is_empty() {
        return Err(ExecutorError::Io(io::Error::other(
            "OpenTeamsCli session.prompt returned empty response body",
        )));
    }
    ...
    if let Some(name) = parsed.get("name").and_then(Value::as_str) {
        let message = parsed
            ...
            .unwrap_or(trimmed);
        return Err(ExecutorError::Io(io::Error::other(format!(
            "OpenTeamsCli session.prompt failed: {name}: {message}"
        ))));
    }

    Err(ExecutorError::Io(io::Error::other(format!(
        "OpenTeamsCli session.prompt returned unexpected response: {trimmed}"
    ))))
}
...
pub async fn session_command(
    ...
    if trimmed.is_empty() {
        return Err(ExecutorError::Io(io::Error::other(
            "OpenTeamsCli session.command returned empty response body",
        )));
    }
    ...
    if let Some(name) = parsed.get("name").and_then(Value::as_str) {
        let message = parsed
            ...
            .unwrap_or(trimmed);
        return Err(ExecutorError::Io(io::Error::other(format!(
            "OpenTeamsCli session.command failed: {name}: {message}"
        ))));
    }

    Err(ExecutorError::Io(io::Error::other(format!(
        "OpenTeamsCli session.command returned unexpected response: {trimmed}"
    ))))
}
...
async fn build_response_error(resp: reqwest::Response, context: &str) -> ExecutorError {
    ...
    ExecutorError::Io(io::Error::other(format!(
        "OpenTeamsCli {context} failed: HTTP {status} {body}"
    )))
}
...
pub async fn resolve_compaction_model(
    ...
    Err(ExecutorError::Io(io::Error::other(
        "OpenTeamsCli compaction requires a configured model",
    )))
}
        ...
        let current_resp = match resp.take() {
            ...
            None => {
                    ...
                    .await
                {
                    ...
                    Err(err) => {
                        let _ = log_writer
                            .log_error(format!("OpenTeamsCli event stream reconnect failed: {err}"))
                            .await;
                        attempt += 1;
...
fn extract_retry_status(event: &Value) -> Option<(u64, String)> {
    ...
    let message = event
        ...
        .and_then(Value::as_str)
        .unwrap_or("OpenTeamsCli request is retrying")
        .to_string();
    ...
    loop {
        ...
        let Ok(data) = serde_json::from_str::<Value>(trimmed) else {
            let _ = ctx
                .log_writer
                .log_error(format!(
                    "OpenTeamsCli event stream delivered non-JSON event payload: {trimmed}"
                ))
                .await;
            continue;
        };
        ...
        let _ = ctx
            .log_writer
            .log_event(&OpenTeamsCliExecutorEvent::SdkEvent {
                event: data.clone(),
            })
            .await;
        ...
        match event_type {
            ...
            "session.status" => {
                ...
                {
                    let message =
                        format!("OpenTeamsCli request exceeded retry limit ({attempt}): {message}");
            ...
            "session.error" => {
                ...
                let message = data
                    ...
                    .and_then(Value::as_str)
                    .unwrap_or("OpenTeamsCli session error")
                    .to_string();
            ...
            "permission.asked" => {
                ...
                tokio::spawn(async move {
                    ...
                    .await
                    {
                        ...
                        Err(err) => {
                            tracing::error!(
                                "OpenTeamsCli approval failed for tool_call_id={}: {err}",
                                tool_call_id
                            );
                            return;
                        }

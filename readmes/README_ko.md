...
## 🔥 *최신 소식:*
### 기능 미리보기
v0.4.0에서는 큰 업데이트를 도입할 예정입니다.
- 현재 상태: 현재의 @mention 기반 자유 협업 모델은 지나치게 개방적입니다. Agent 출력은 본질적으로 불확실성이 있기 때문에 협업이 불안정해질 수 있고, 협업 체인이 특정 지점에서 끊어질 수 있습니다.
- 개선 방향: 협업 효율을 높이기 위해 워크플로 모드를 도입하려고 합니다. 이 모드는 Claude Agent Teams와 유사한 중앙 집중형 커뮤니케이션 구조를 사용합니다. 각 작업은 하나의 워크플로 노드가 되고, 모든 노드가 함께 작업 흐름 DAG를 구성하며, 독립 스케줄러가 워크플로 상태 전이를 관리합니다. 복잡한 작업에서는 한 번의 계획만으로 요구 사항을 완전히 만족하지 못할 수 있으므로, 워크플로는 두 번째 또는 세 번째 반복까지 지원해야 합니다. 이를 위해 워크플로에는 설정 가능한 반복 횟수와 반복 이력 조회 기능이 포함됩니다.
- 이번 변경이 OpenTeams의 실용성을 크게 높일 것이라고 믿습니다. 기대해 주세요.

<img width="1100" height="546" alt="Image" src="https://github.com/user-attachments/assets/be065bc4-f736-47a8-a989-05e5142264f8" />

### *주요 업데이트*
- **2026.05.07 (v0.3.22)**
  - 그룹 채팅 세션의 멤버를 한 번의 클릭으로 프리셋 팀으로 저장하는 기능 지원
- **2026.04.14 (v0.3.15)**
  - 워크스페이스 파일 변경 뷰어
- **2026.04.06 (v0.3.12)**
  - 다크 UI 모드 활성화

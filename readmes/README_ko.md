...
**openteams**는 오픈 소스 멀티 에이전트 협업 워크스페이스입니다. Claude Code, Codex, Gemini CLI 같은 여러 AI 코딩 에이전트를 하나의 공유 세션으로 가져와 서로 소통하고, 컨텍스트를 공유하며, 팀처럼 함께 일하게 합니다. 가벼운 Free Chat으로 협업하거나, 보이는 계획, 단계별 제어, 추적 가능한 리뷰가 포함된 구조화된 워크플로우로 복잡한 작업을 오케스트레이션할 수 있습니다. 선택형 격리 워크스페이스는 세션마다 별도의 Git worktree를 제공해 에이전트가 서로 방해하지 않고 독립적인 작업을 수행하게 합니다. openteams는 실행을 넘어 아이디어부터 출시까지 전체 과정을 관리합니다. 이슈로 진행 상황과 우선순위를 확인하고, GitHub 작업을 동기화하며, 각 이슈를 실제 실행 세션과 연결할 수 있습니다. 작업이 끝나면 빌드 통계가 결과를 세션, 모델, 작업별 Token 사용량 및 비용과 연결해 산출물과 효율을 명확히 보여 줍니다. 모든 것은 사용자의 로컬 워크스페이스에서 실행됩니다.
...
**공유 워크스페이스에서는 독립적인 작업도 충돌할 수 있습니다.** 서로 다른 세션이 같은 파일을 동시에 수정하면 완료되지 않은 변경이 다른 작업에 섞이고, 에이전트가 서로 방해하며, 각 결과를 따로 검토하거나 병합하기 어려워집니다.

**에이전트 중심 개발은 프로젝트 전체 방향을 놓치게 만들 수 있습니다.** 한 기능을 끝낸 뒤 다음 할 일이 머릿속이나 흩어진 프롬프트에만 남아 있을 수 있습니다. 모든 작업이 새 채팅에서 시작되면 전체 로드맵과 우선순위를 보기 어렵고, 프로젝트가 일관된 출시를 향해 가는지 판단하기 어렵습니다.

**Token 사용량은 세기 쉽지만 실제 가치와 연결하기 어렵습니다.** Token은 에이전트, 세션, 모델에 걸쳐 소비되지만 총사용량만으로는 수정한 버그 수나 제공한 기능 수를 알 수 없습니다. 비용과 결과를 연결하지 않으면 에이전트 기반 개발이 실제로 더 효율적이 되는지 판단할 수 없습니다.

**openteams**는 개발 과정 전체에 명확성과 통제력을 제공합니다. 같은 세션의 에이전트는 하나의 컨텍스트를 공유하므로 반복해서 전환하거나 설명할 필요가 없습니다. 복잡한 작업은 **보이고 제어 가능한 워크플로우**가 되어, 실행 전에 계획을 다듬고 각 단계를 지켜보며 어떤 노드에서든 승인, 거절, 재시도, 방향 전환을 할 수 있습니다.

격리 워크스페이스는 세션마다 별도의 Git worktree를 제공해 완료되지 않은 변경을 공유하지 않고 독립적인 작업을 수행하게 합니다. 각 세션 결과를 따로 검토한 뒤 원하는 시점에 병합하거나 폐기할 수 있습니다.

이슈 관리는 프로젝트 로드맵을 다시 개발자의 손에 둡니다. 필요한 작업을 기록하고 우선순위를 정한 뒤, 이슈에서 바로 가벼운 실행 세션을 만들거나 연결할 수 있습니다. 이슈는 에이전트가 임의로 바꾸지 않고 개발자가 관리하므로, 다음 할 일과 전체 진행 상황에 대해 항상 직접 소유하는 신뢰할 수 있는 기준을 갖게 됩니다.

빌드 통계는 투입과 결과 사이의 피드백 루프를 완성합니다. 이번 주에 수정한 버그 수, 제공한 기능 수, 소비한 Token 수를 세션 및 모델별 내역과 함께 보여 줍니다. 얼마나 사용했는지뿐 아니라 그 비용으로 무엇을 만들었는지도 알 수 있습니다.

> 진짜 레버리지는 에이전트를 더 많이 두는 것이 아니라, 무엇을 맡기고 어떻게 실행하며 그 결과가 비용에 맞는지 계속 통제하는 것입니다.

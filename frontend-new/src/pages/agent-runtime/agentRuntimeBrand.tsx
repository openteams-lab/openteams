...
export const agentBrandMarks: Record<BaseCodingAgent, AgentBrandMark> = {
  AMP: {
    title: "Amp",
...
export function AgentBrandAvatar({
  ...
  const maskStyle =
    brand.logoSrc && brand.logoMode === "mask"
      ...
      : undefined;
  const resolvedIconClassName = iconClassName ?? brand.logoClassName;
  ...
  return (
    ...
    >
      {brand.logoSrc && brand.logoMode === "mask" ? (
        <span
          aria-hidden="true"
          className={cx("block shrink-0 bg-current", resolvedIconClassName ?? "h-5 w-5")}
          style={maskStyle}
        />
      ) : brand.logoSrc ? (
        <img
          ...
          alt=""
          aria-hidden="true"
          className={cx("object-contain", resolvedIconClassName ?? "h-5 w-5")}
        />
      ) : (
        <svg
          aria-hidden="true"

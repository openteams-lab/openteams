...
<Step title="Authenticate with Factory AI">
  To use Droid in agents-chatgroup, you must first authenticate:
...
Droid supports several configuration options in agents-chatgroup:

- **Autonomy Level**: Controls permission level for file and system operations
  - `normal`: Read-only mode 鈥?safe for reviewing planned changes without execution
...
These options can be configured when creating agent configurations in agents-chatgroup.

...
description: "Common issues and solutions when using agent-chatgroup"
---
...
```bash
RUST_LOG=debug npx agent-chatgroup
```
...
If you encounter irrecoverable errors and need to completely wipe your agent-chatgroup database, you can delete the application data directory for your operating system. 
...
<Tab title="macOS">
```bash
~/Library/Application Support/ai.starterra.ai.agent-chatgroup/
```
</Tab>

<Tab title="Linux">
```bash
~/.local/share/agent-chatgroup/
```
</Tab>

<Tab title="Windows">
```
%APPDATA%\starterra.ai\agent-chatgroup\
```

Typically: `C:\Users\<username>\AppData\Roaming\starterra.ai\agent-chatgroup\`
</Tab>
</Tabs>

After deleting the application data directory, restart agent-chatgroup to reset with an empty database and default settings.

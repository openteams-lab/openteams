...
<Tip>
The dev server must output its URL (e.g., `http://localhost:3000`) to stdout for agent-chatgroup to detect and display it in the preview panel.
</Tip>

<Card title="Preview Panel Documentation" icon="eye" href="/workspaces/preview">
...
<Warning>
Cleanup scripts should be idempotent鈥攕afe to run even if the resources don't exist. Use `|| true` to prevent failures when there's nothing to clean up.
</Warning>
...
Run your scripts manually in a terminal before configuring them in agent-chatgroup to ensure they work correctly.
</Accordion>
</AccordionGroup>

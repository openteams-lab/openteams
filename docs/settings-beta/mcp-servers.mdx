...
description: "Configure MCP (Model Context Protocol) servers to enhance your coding agents within agent-chatgroup with additional tools and capabilities."
---
...
<Note>
This page covers configuring MCP servers **within** agent-chatgroup for your coding agents. For connecting external MCP clients to agent-chatgroup's MCP server, see the [agent-chatgroup MCP Server](/integrations/agent-chatgroup-mcp-server) guide.
</Note>

## Overview

MCP servers provide additional functionality to coding agents through standardized protocols. You can configure different MCP servers for each coding agent in agent-chatgroup, giving them access to specialized tools like browser automation, access to remote logs, error tracking via Sentry, or documentation from Notion.
...
<Frame>
<img src="/images/settings-beta-mcp.png" alt="MCP Server configuration page showing agent selection, JSON configuration, and popular servers" />
</Frame>

## Popular MCP Servers

agent-chatgroup provides one-click installation for popular MCP servers. Click a card to insert a pre-configured MCP server into the JSON configuration.

<Frame>
<img src="/images/settings-beta-popular-mcp-servers.png" alt="Popular MCP servers including agent-chatgroup, Context7, Playwright, Exa, Chrome DevTools, and Dev Manager" />
</Frame>
...
<Check>
These changes update the global configuration file of the coding agent and will persist even if you stop using agent-chatgroup.
</Check>
</Step>
</Steps>
...
- Explore the [Agent Configurations](/settings-beta/agent-configurations) guide for advanced agent setup

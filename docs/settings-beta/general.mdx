...
<Frame>
<img src="/images/settings-beta-general.png" alt="General settings tab showing appearance, default agent, and editor options" />
</Frame>
...
<Frame>
<img src="/images/settings-beta-open-in-ide.png" alt="Open in IDE option in the context bar" />
</Frame>
...
When running agent-chatgroup on a remote server (e.g., accessed via Cloudflare tunnel, ngrok, or as a systemctl service), you can configure VSCode-based editors to open projects via SSH instead of assuming localhost.
...
- agent-chatgroup runs on a remote server (VPS, cloud instance, etc.)
...
Set a prefix for auto-generated branch names. When you create a new workspace or task, agent-chatgroup automatically creates a git branch for your changes.
...
Specify where agent-chatgroup stores workspace data. Workspaces are created in a `.agent-chatgroup-workspaces` subdirectory within this path.
...
<Frame>
<img src="/images/settings-beta-notifications.png" alt="Notifications settings showing sound effects and push notifications toggles" />
</Frame>

Configure how agent-chatgroup alerts you about task progress and status changes.

- **Sound Effects** - Play audio notifications when tasks complete, need attention, or encounter errors. Useful when working with multiple tasks or when agent-chatgroup runs in a background tab.
- **Push Notifications** - Receive browser notifications even when agent-chatgroup isn't in focus. Requires browser permission when first enabled.
...
Enable or disable telemetry data collection to help improve agent-chatgroup.

## Message Input

Choose the keyboard shortcut to send messages in the chat input (`Enter` or `鈱?Ctrl + Enter`).

---
title: "agent-chatgroup MCP Server"
description: "Configure the agent-chatgroup MCP server"
---

agent-chatgroup exposes a local MCP (Model Context Protocol) server, allowing you to manage projects and tasks from external MCP clients like Claude Desktop, Raycast, or even coding agents running within agent-chatgroup itself.

<Note>
This page covers connecting **external MCP clients** to agent-chatgroup's MCP server. For configuring MCP servers **within** agent-chatgroup for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
agent-chatgroup's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the agent-chatgroup MCP server to any [supported coding agent](/supported-coding-agents) **within** agent-chatgroup.

1. In agent-chatgroup Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the agent-chatgroup card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.jpeg" alt="MCP Servers configuration page showing how to add agent-chatgroup MCP server" />
</Frame>
...
```json
{
  "mcpServers": {
    "agent_chatgroup": {
      "command": "npx",
      "args": ["-y", "agent-chatgroup@latest", "--mcp"]
    }
  }
}
```
...
The agent-chatgroup MCP server provides the following tools for managing projects, tasks, and task execution:
...
3. **Automatic Task Generation**: Your MCP client will use the agent-chatgroup MCP server to automatically create structured tasks in your project
...
Your MCP client will use the `create_task` tool to break this down into individual tasks and add them to your agent-chatgroup project automatically.
...
### Internal Coding Agents (Within agent-chatgroup)

A powerful workflow involves using coding agents within agent-chatgroup that are also connected to the agent-chatgroup MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual tasks directly in the agent-chatgroup interface
...
Raycast is a popular MCP client that can connect to agent-chatgroup's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding agent-chatgroup server" />
</Frame>

Configure the agent-chatgroup MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the agent-chatgroup MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>

...
agent-chatgroup Cloud lets you collaborate with your team in real-time. Your data syncs to the cloud, so everyone sees the same projects, issues, and updates.

<Info>
**Already using agent-chatgroup locally?** Cloud uses the same interface you're familiar with. The difference is your data syncs to the cloud and you can invite team members.
</Info>

## Step 1: Launch agent-chatgroup

If you haven't already, start agent-chatgroup:
...
<Step title="Authorise the application">
  You'll be redirected to GitHub or Google. Sign in if needed, then click **Authorize** or **Allow** to grant agent-chatgroup access.

  <Info>
  agent-chatgroup only requests access to your basic profile (name, email, avatar). It cannot access your private repositories or post on your behalf.
  </Info>
</Step>

<Step title="Return to agent-chatgroup">
...
You now have:
- 鉁?A Cloud account
- 鉁?An organisation for your team
- 鉁?A project with a kanban board
...
Both appear in the same agent-chatgroup interface. Local projects show in one section, Cloud projects in another.

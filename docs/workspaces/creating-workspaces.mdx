...
agent-chatgroup creates a **git worktree** - a separate working directory with its own branch. This keeps your changes isolated from your main codebase. Your original repository remains untouched.
</Step>
...
<Info>
**Where do workspaces live?** Worktrees are created in a `.agent-chatgroup-workspaces` directory (configurable in Settings 鈫?General 鈫?Workspace Directory). Each workspace gets its own folder.
</Info>
...
- The command bar: `Cmd/Ctrl + K` 鈫?**Start Dev Server**
...
1. Press `Cmd/Ctrl + K`
2. Go to **Workspace Actions** 鈫?**Archive**
...
3. Check that agent-chatgroup has permission to access the folder
</Accordion>
...
2. Verify API keys are configured in Settings 鈫?Agents

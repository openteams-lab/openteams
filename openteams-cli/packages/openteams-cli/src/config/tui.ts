...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.OPENTEAMS_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.OPENTEAMS_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.OPENTEAMS_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".openteams") && dir !== Flag.OPENTEAMS_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old openteams.json shape) still get their settings applied.
    const normalized = (() => {

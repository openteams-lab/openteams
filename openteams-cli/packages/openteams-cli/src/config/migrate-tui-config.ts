...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from openteams.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const openteams = await openteamsFiles(input)
  for (const file of openteams) {
...
async function openteamsFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.OPENTEAMS_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("openteams-cli", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "openteams-cli")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "openteams-cli"))
  }
  if (Flag.OPENTEAMS_CONFIG) files.push(Flag.OPENTEAMS_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "openteams-cli"))

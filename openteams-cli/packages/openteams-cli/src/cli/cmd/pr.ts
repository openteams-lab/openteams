...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run openteams-cli",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for openteams-cli session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found openteams-cli session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["openteams-cli", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting openteams-cli...")
        UI.println()

        const openteamsArgs = sessionId ? ["-s", sessionId] : []
        const openteamsProcess = Process.spawn(["openteams-cli", ...openteamsArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await openteamsProcess.exited
        if (code !== 0) throw new Error(`openteams-cli exited with code ${code}`)
      },
    })
  },
})

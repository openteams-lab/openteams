    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: openteams-cli mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in openteams.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .openteams/ subdirectory too)
  const candidates = [path.join(baseDir, "openteams.json"), path.join(baseDir, "openteams.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".openteams", "openteams.json"), path.join(baseDir, ".openteams", "openteams.jsonc"))
  }
  ...
  // Default to openteams.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., openteams-cli x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "openteams-cli-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })

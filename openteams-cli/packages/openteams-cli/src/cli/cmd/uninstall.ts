...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall openteams-cli and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenTeams CLI")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g openteams-cli",
      pnpm: "pnpm uninstall -g openteams-cli",
      bun: "bun remove -g openteams-cli",
      yarn: "yarn global remove openteams-cli",
      brew: "brew uninstall openteams-cli",
      choco: "choco uninstall openteams-cli",
      scoop: "scoop uninstall openteams-cli",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "openteams-cli", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenTeams CLI!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# openteams-cli") || content.includes(".openteams/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".openteams/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".openteams/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".openteams"))
    ) {
      continue
    }

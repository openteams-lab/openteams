...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@openteams/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `openteams-cli auth login` in the terminal",
        name: "Login with openteams-cli",
        id: "openteams-cli-login",
      }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const openteamsProvider = providers.find((p) => p.id === "openteams-cli")
    if (openteamsProvider) {
      if (openteamsProvider.models["big-pickle"]) {
        return { providerID: ProviderID.openteams, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(openteamsProvider.models))
      if (best) {
        return {

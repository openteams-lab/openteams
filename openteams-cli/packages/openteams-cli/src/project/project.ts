...
import { fn } from "@openteams/util/fn"
...
export namespace Project {
  ...
  function readCachedId(dir: string) {
    return Filesystem.readText(path.join(dir, "openteams-cli"))
      .then((x) => x.trim())
      .then(ProjectID.make)
      .catch(() => undefined)
  }
  ...
  export async function fromDirectory(directory: string) {
      ...
      if (dotgit) {
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox,
            vcs: Info.shape.vcs.parse(Flag.OPENTEAMS_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          if (id) {
            ...
            await Filesystem.write(path.join(worktree, ".git", "openteams-cli"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.OPENTEAMS_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)

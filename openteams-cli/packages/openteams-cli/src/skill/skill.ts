...
export namespace Skill {
  ...
  const OPENTEAMS_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
  ...
  const create = (instance: InstanceContext.Shape, discovery: Discovery.Interface): Cache => {
    ...
    const load = async () => {
      if (!Flag.OPENTEAMS_DISABLE_EXTERNAL_SKILLS) {
      ...
      for (const dir of await Config.directories()) {
        await scan(state, dir, OPENTEAMS_SKILL_PATTERN)
      }
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@openteams/Skill") {}

...
declare global {
  const OPENTEAMS_VERSION: string
  const OPENTEAMS_CHANNEL: string
}
...
export namespace Installation {
  ...
  export async function method() {
    ...
    const checks = [
      ...
      {
        name: "brew" as const,
        command: () => text(["brew", "list", "--formula", "openteams-cli"]),
      },
      {
        name: "scoop" as const,
        command: () => text(["scoop", "list", "openteams-cli"]),
      },
      {
        name: "choco" as const,
        command: () => text(["choco", "list", "--limit-output", "openteams-cli"]),
      },
    ]
    ...
    for (const check of checks) {
      ...
      const installedName =
        check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "openteams-cli" : "openteams-cli"
  ...
  async function getBrewFormula() {
    const tapFormula = await text(["brew", "list", "--formula", "anomalyco/tap/openteams-cli"])
    if (tapFormula.includes("openteams-cli")) return "anomalyco/tap/openteams-cli"
    const coreFormula = await text(["brew", "list", "--formula", "openteams-cli"])
    if (coreFormula.includes("openteams-cli")) return "openteams-cli"
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      case "curl":
        ...
        break
      case "npm":
        result = await Process.run(["npm", "install", "-g", `openteams-cli@${target}`], { nothrow: true })
        break
      case "pnpm":
        result = await Process.run(["pnpm", "install", "-g", `openteams-cli@${target}`], { nothrow: true })
        break
      case "bun":
        result = await Process.run(["bun", "install", "-g", `openteams-cli@${target}`], { nothrow: true })
        break
      case "brew": {
      ...
      case "choco":
        result = await Process.run(["choco", "upgrade", "openteams-cli", `--version=${target}`, "-y"], { nothrow: true })
        break
      case "scoop":
        result = await Process.run(["scoop", "install", `openteams-cli@${target}`], { nothrow: true })
        break
      default:
  ...
  export const VERSION = typeof OPENTEAMS_VERSION === "string" ? OPENTEAMS_VERSION : "local"
  export const CHANNEL = typeof OPENTEAMS_CHANNEL === "string" ? OPENTEAMS_CHANNEL : "local"
  export const USER_AGENT = `openteams-cli/${CHANNEL}/${VERSION}/${Flag.OPENTEAMS_CLIENT}`
  ...
  export async function latest(installMethod?: Method) {
    ...
    if (detectedMethod === "brew") {
      ...
      return fetch("https://formulae.brew.sh/api/formula/openteams.json")
        .then((res) => {
          if (!res.ok) throw new Error(res.statusText)
          return res.json()
        })
    ...
    if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
      ...
      return fetch(`${registry}/openteams-cli/${channel}`)
        .then((res) => {
          if (!res.ok) throw new Error(res.statusText)
          return res.json()
        })
    ...
    if (detectedMethod === "choco") {
      return fetch(
        "https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27openteams-cli%27%20and%20IsLatestVersion&$select=Version",
    ...
    if (detectedMethod === "scoop") {
      return fetch("https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/openteams.json", {
    ...
    return fetch("https://api.github.com/repos/anomalyco/openteams-cli/releases/latest")
      .then((res) => {
        if (!res.ok) throw new Error(res.statusText)
        return res.json()
      })

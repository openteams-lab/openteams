...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const openteamsModel = {
      ...openaiModel,
      providerID: "openteams-cli",
      api: {
        id: "openteams-cli-test",
        url: "https://api.openteams.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, openteamsModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.openteams?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.openteams?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          openteams: { itemId: "msg_openteams" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              openteams: { itemId: "msg_openteams_part" },
    ...
    expect(result[0].providerOptions?.openteams?.itemId).toBe("msg_openteams")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.openteams?.itemId).toBe("msg_openteams_part")

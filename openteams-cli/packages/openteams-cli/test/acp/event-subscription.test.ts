...
import type { Event, EventMessagePartUpdated, ToolStatePending, ToolStateRunning } from "@openteams/sdk/v2"
...
function createFakeAgent() {
  ...
  const agent = new ACP.Agent(connection, {
    sdk,
    defaultModel: { providerID: "openteams-cli", modelID: "big-pickle" },
  } as any)
  ...
  test("routes message.part.delta by the event sessionID (no cross-session pollution)", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const cwd = "/tmp/openteams-cli-acp-test"

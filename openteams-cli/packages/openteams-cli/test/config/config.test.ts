...
const managedConfigDir = process.env.OPENTEAMS_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "openteams.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "openteams.json") {
...
test("ignores legacy tui keys in openteams-cli config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://opencode.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "openteams.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["OPENTEAMS_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      openteams: {
        options: {
          apiKey: "{env:OPENTEAMS_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["openteams-cli"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["OPENTEAMS_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["OPENTEAMS_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "openteams.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .openteams directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openteamsDir = path.join(dir, ".openteams")
      await fs.mkdir(openteamsDir, { recursive: true })
      const agentDir = path.join(openteamsDir, "agent")
...
test("loads agents from .openteams/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openteamsDir = path.join(dir, ".openteams")
      await fs.mkdir(openteamsDir, { recursive: true })

      const agentsDir = path.join(openteamsDir, "agents")
...
test("loads commands from .openteams/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openteamsDir = path.join(dir, ".openteams")
      await fs.mkdir(openteamsDir, { recursive: true })

      const commandDir = path.join(openteamsDir, "command")
...
test("loads commands from .openteams/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openteamsDir = path.join(dir, ".openteams")
      await fs.mkdir(openteamsDir, { recursive: true })

      const commandsDir = path.join(openteamsDir, "commands")
...
test("does not try to install dependencies in read-only OPENTEAMS_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.OPENTEAMS_CONFIG_DIR
  process.env.OPENTEAMS_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OPENTEAMS_CONFIG_DIR
    else process.env.OPENTEAMS_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OPENTEAMS_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "openteams.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .openteams config
      const projectDir = path.join(dir, "project")
      const openteamsDir = path.join(projectDir, ".openteams")
      await fs.mkdir(openteamsDir, { recursive: true })
      ...
      // Local .openteams config with different plugins
      await Filesystem.write(
        path.join(openteamsDir, "openteams.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openteams config with some overlapping plugins
      await Filesystem.write(
        path.join(openteamsDir, "openteams.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets OPENTEAMS_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .openteams config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openteams directory config enables it
      const openteamsDir = path.join(dir, ".openteams")
      await fs.mkdir(openteamsDir, { recursive: true })
      await Filesystem.write(
        path.join(openteamsDir, "openteams.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/openteams-cli")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/openteams-cli")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-openteams-cli@2.4.3")).toBe("oh-my-openteams-cli")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@openteams/plugin@2.0.0")).toBe("@openteams/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-openteams-cli@2.4.3", "file:///project/.openteams/plugin/oh-my-openteams-cli.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.openteams/plugin/oh-my-openteams-cli.js")
  })
  ...
  test("local plugin directory overrides global openteams.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const openteamsDir = path.join(projectDir, ".openteams")
        const pluginDir = path.join(openteamsDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENTEAMS_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"]
    process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .openteams/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"]
    process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .openteams directory with a command
          const openteamsDir = path.join(dir, ".openteams", "command")
          await fs.mkdir(openteamsDir, { recursive: true })
          await Filesystem.write(path.join(openteamsDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .openteams should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENTEAMS_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OPENTEAMS_CONFIG_DIR"]
      process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OPENTEAMS_CONFIG_DIR"]
      } else {
        process.env["OPENTEAMS_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OPENTEAMS_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENTEAMS_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OPENTEAMS_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OPENTEAMS_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from OPENTEAMS_CONFIG_DIR, not project
...
describe("OPENTEAMS_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENTEAMS_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENTEAMS_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["OPENTEAMS_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OPENTEAMS_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OPENTEAMS_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OPENTEAMS_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENTEAMS_CONFIG_CONTENT"]
